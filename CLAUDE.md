...
2. `src/check.rs` -- Core logic: vendored crates.io validation, reserved names (56 entries from 3 DB migrations), crates.io API lookups via ureq. Public types: `Availability`, `CheckError`, `Client`. `canon_crate_name()` normalizes hyphens/underscores.
...
- Unit tests in `src/check.rs` (canonicalization, reserved names, validation)
